...
import io.stallion.dal.db.converters.*;
import io.stallion.dal.db.mysql.MySqlTickets;
import io.stallion.dal.db.postgres.PostgresTickets;
...
public class DB {
    ...
    public static DB load() {
        if (empty(Settings.instance().getDatabase()) || empty(Settings.instance().getDatabase().getUrl())) {
            return null;
        }
        ...
        DB db;
        if (empty(config.getDbAccessorClass()) || "io.stallion.dal.db.DB".equals(config.getDbAccessorClass())) {
            db = new DB();
        } else {
