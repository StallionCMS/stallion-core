...
public class StandardModelController<T extends Model> implements ModelController<T> {
    ...
    protected void postInitialize(DalRegistration registration) {
    ...
    public T mergeUpdates(T target, T newInfo, String...fields) {
        for (String field: fields) {
            PropertyUtils.setProperty(target, field, PropertyUtils.getPropertyOrMappedValue(newInfo, field));
        }
        return target;
    }
    ...
    public T mergeDetached(Long id, T toMergeIn, Class<Annotation> anno) {
        return getStash().mergeDetached(id, toMergeIn, anno);
    }
    ...
    @Override
    public T mergeWithDefaults(T toMergeIn) {
        return mergeWithDefaults(toMergeIn, null);
    }

    @Override
    public T mergeWithDefaults(T toMergeIn, Class<Annotation> anno) {
        return getStash().mergeWithDefaults(toMergeIn, anno);
    }
