...
public class LocalMemoryStash<T extends Model> extends StashBase<T> {
    ...
    private void initialize() {
        ...
        for(String propertyName: PropertyUtils.getPropertyNames(this.getPersister().getModelClass())) {
            ...
            if (PropertyUtils.propertyHasAnnotation(this.getPersister().getModelClass(), propertyName, Key.class)) {
                Log.fine("Model:{0} has alternativeKey on {1}", this.getPersister().getModelClass(), propertyName);
                this.keyNameToKeyToValue.put(propertyName, new HashMap<Object, Set<T>>());
            }
    ...
    @Override
    public void syncAllFields(T obj) {
        Model existing = this.itemByPrimaryKey.getOrDefault(obj.getId(), null);
        List<String> changedKeyFields = new ArrayList<>();
    ...
    @Override
    public T mergeDetached(Long id, T obj) {
        return mergeDetached(id, obj, null);
    }

    @Override
    public T mergeDetached(Long id, T toMergeIn, Class<Annotation> anno) {
        T existing = forId(id);
        if (existing == null) {
            throw new NotFoundException("The object with id " + id + " from the bucket " + getBucket() + " was not found.");
        }
        // Merge in all non-null, non-empty properties of the merged object
        for(Map.Entry entry: PropertyUtils.getProperties(toMergeIn).entrySet()) {
            if (entry.getValue() == null) {
                continue;
            }
            if (anno != null) {
                Method getter = PropertyUtils.getGetter(existing, entry.getKey().toString());
                if (!getter.isAnnotationPresent(anno)) {
                    continue;
                }
            }
            PropertyUtils.setProperty(existing, entry.getKey().toString(), entry.getValue());
        }
        return existing;
    }

    @Override
    public T mergeWithDefaults(T obj) {
        return mergeWithDefaults(obj, null);
    }

    @Override
    public T mergeWithDefaults(T toMergeIn, Class<Annotation> anno) {
        T existing = null;
        try {
            existing = (T) getPersister().getModelClass().newInstance();
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        // Merge in all non-null, non-empty properties of the merged object
    ...
    public void save(T obj) {
        ...
        if (existing == null) {
            if (empty(obj.getId())) {
                obj.setId(DalRegistry.instance().getTickets().nextId());
