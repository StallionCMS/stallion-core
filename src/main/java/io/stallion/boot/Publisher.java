...
import io.stallion.settings.childSections.PublishingConfig;
 ...
 *
 */
class Publisher implements StallionRunAction<PublishingCommandOptions> {
    private PublishingConfig config;
    private String deployEnv = "";
    private PublishingCommandOptions options;
    ...
    public String getHelp() {
        return "Publishes the stallion site to linux server host defined in the publishing config section of the stallion.toml";
    }

    @Override
    public void loadApp(PublishingCommandOptions options) {
        AppContextLoader.loadWithSettingsOnly(options);
    }

    @Override
    public PublishingCommandOptions newCommandOptions() {
        return new PublishingCommandOptions();
    }

    @Override
    public void execute(PublishingCommandOptions options) throws Exception {
        this.options = options;
        publish((PublishingCommandOptions)options);
    }

    public void publish(PublishingCommandOptions options) throws IOException, CommandException {
        List<PublishingConfig> configs = Context.getSettings().getPublishing();
        ...
        if (configs.size() == 0) {
            ...
            for (PublishingConfig aConfig : configs) {
    ...
    private void doPublish(String host) throws IOException, CommandException {
        ...
        if (config.getRedirectToPrimary()) {
            redirectToPrimary = 1;
        }
        ...
        Map conf = map(
                ...
                val("secondary_domains", config.getSecondaryDomains()),
