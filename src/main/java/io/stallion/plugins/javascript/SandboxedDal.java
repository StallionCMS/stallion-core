...
public class SandboxedDal implements Map<String, ModelController> {

    private Sandbox sandbox;

    public SandboxedDal(Sandbox sandbox) {
        this.sandbox = sandbox;
    }

    @Override
    public int size() {
        return DalRegistry.instance().size();
    }
    ...
    public boolean containsKey(Object key) {
        return DalRegistry.instance().containsKey(key);
    }
    ...
    public ModelController get(Object key) {
        if (sandbox.isCanWriteAllData()) {
            return DalRegistry.instance().get(key);
        }
        ...
        if (sandbox.isCanReadAllData()) {
            return DalRegistry.instance().get(key).getReadonlyWrapper();
        }
