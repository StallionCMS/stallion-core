...
public class QueryToPager<T extends Model> {
    ...
    public Pager<T> pager() {
        ...
        if (!empty(request.getQueryParam("page"))) {
            page = Integer.parseInt(request.getQueryParam("page"));
        }
    ...
    protected void process() {
        ...
        String search = request.getQueryParam("search", null);
        if (!empty(search)) {
        ...
        String filters = request.getQueryParam("filters", null);
        if (!empty(filters)) {
        ...
        String sort = or(request.getQueryParam("sort", null), defaultSort);
        if (!empty(sort)) {
