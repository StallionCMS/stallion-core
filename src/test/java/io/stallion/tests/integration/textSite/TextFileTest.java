...
public class TextFileTest extends JerseyIntegrationBaseCase {
    ...
    public void testPageParsing() {
        ...
        Response response = GET("/toml-meta-things");
    ...
    public void testRawHtml() {
        ...
        Response response = GET("/with-raw-html");
        String out = response.readEntity(String.class);
        assertContains(out, expectedRawHtml);
        assertContains(out, secondExpectedRawHtml);
        assertContains(out, secondExpectedRawHtml);
        assertContains(out, "To wash my face clean after the dust of the road, and to drink, so the dry bread will not stick in my throat.");
    ...
    public void testOnePage() {

        {
            ...
            String out = response.readEntity(String.class);
            assertContains(out, "<h1>Frequently Asked Questions</h1>");
        }

        {
            ...
            String out = response.readEntity(String.class);
            assertContains(out, "This is the home page for the text_site page collection.");
        }
    ...
    public void testPageVisible()
    {
        ...
        {
            Response response = GET("/draft-page");
            assertResponseDoesNotContain(response, "The Draft Page", 404);
        }

        {
            Response response = GET("/future-dated");
